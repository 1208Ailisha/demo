...
public class ZkClientTest {
	public static void main(String[] args) {
		...
		/**
		 * 验证childChangesListener
		 */
		String newNode = node.concat("/").concat("new");
		if(zkClient.exists(newNode)){
			zkClient.delete(newNode);
			zkClient.createPersistent(newNode, "hello zookeeper,new node");
		}
		
		System.out.println("*****************修改前："+zkClient.readData(node));
		...
		System.out.println("*****************修改后："+zkClient.readData(node));
	...
	public static void childChangesListener(ZkClient zkClient, final String path) {
		zkClient.subscribeChildChanges(path, new IZkChildListener() {
			...
			public void handleChildChange(String parentPath,
				...
				System.out.println("*****************clildren of path " + parentPath + ":"
						+ currentChilds);
			}
