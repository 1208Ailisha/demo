...
public class ZkDistributedQueue {
				...
				try {
					if (getLetterNum() == MAILBOX_MAX_SIZE) { // 信箱已满
						...
						String newLetterPath = zkClient.create(
								...
								"letter".getBytes(), Ids.OPEN_ACL_UNSAFE,
								CreateMode.PERSISTENT_SEQUENTIAL);
						System.out.println("a new letter has been received: "
								+ newLetterPath.substring(MAILBOX_ROOT_PATH
										.length() + 1) + ", letter num: "
								+ getLetterNum());
					}
				} catch (Exception e) {
	...
	static class Consumer extends Thread {
			...
			while (true) {
				try {
					if (getLetterNum() == 0) { // 信箱已空
						...
						System.out.println("a letter has been delivered: "
								+ firstLetter + ", letter num: "
								+ getLetterNum());
					}
				} catch (Exception e) {
					System.out
							.println("consumer equit task becouse of exception !");
					e.printStackTrace();
					break;
				}
			}
		...
		private String getFirstLetter() throws KeeperException,
				InterruptedException {
			List<String> letterPaths = zkClient.getChildren(MAILBOX_ROOT_PATH,
					false);
